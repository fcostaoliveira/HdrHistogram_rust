...
pub struct PackedHistogram {
    ...
    // 1-entry last-hit cache: the flat counts-index touched by the previous record
    // and its position in `idx`. `last_index < 0` is the empty sentinel. i64 covers
    // the full u32 range of a flat index without overflow.
    last_index: i64,
    last_pos: usize,
}
...
impl PackedHistogram {
    ...
    /// Add `delta` to the count at flat index `ci`, inserting a new populated
    /// bucket if needed. Returns the final position of `ci` in `idx`.
    fn sparse_add(&mut self, ci: u32, delta: u64) -> usize {
    ...
    pub fn record_n(&mut self, value: u64, count: u64) -> Result<(), RecordError> {
        ...
        if count != 0 {
            let k = index as u32;
            // Fast path: a burst of records hitting the same flat index skips the
            // binary search. The `idx[last_pos] == k` re-check makes the cache
            // robust to positions shifted by an intervening insert.
            if self.last_index >= 0
                && self.last_index as u64 == u64::from(k)
                && self.last_pos < self.idx.len()
                && self.idx[self.last_pos] == k
            {
                // Same width-aware increment + overflow-widen as sparse_add's
                // found branch. Widening leaves idx positions untouched, so
                // last_pos stays valid.
                let pos = self.last_pos;
                let nv = self.slot_get(pos).saturating_add(count);
                self.widen_to_fit(nv);
                self.slot_set(pos, nv);
            } else {
                let pos = self.sparse_add(k, count);
                self.last_index = i64::from(k);
                self.last_pos = pos;
            }
...
mod packed_serialization {
    ...
    use crate::serialization::{
        V2_COMPRESSED_COOKIE, V2_COOKIE, V2_HEADER_SIZE, varint_read_slice, varint_write,
        ...
        fn deser_v2<R: Read>(reader: &mut R) -> Result<Self, PackedDeserializeError> {
            ...
            while i < payload_len {
                ...
                if val < 0 {
                    ...
                    if val != 0 {
                        let _ = p.sparse_add(dst as u32, val as u64);
...
mod fuzz_tests {
    ...
    fn fuzz_differential() {
        ...
        for _ in 0..4000 {
            ...
            for _ in 0..n {
                let v = rng.random_range(1..3_600_000_000u64);
                let c = rng.random_range(1..100_000u64);
                d.record_n(v, c).unwrap();
                p.record_n(v, c).unwrap();
            }
            ...
            for _ in 0..8 {
                let q = rng.random_range(0.0..100.0);
    ...
    fn fuzz_hostile_decode() {
        ...
        for _ in 0..200_000 {
            let mut data: Vec<u8> = if rng.random_bool(0.3) {
                valid.clone()
            } else {
            ...
            if !data.is_empty() && rng.random_bool(0.5) {
                let i = rng.random_range(0..data.len());
                data[i] = rng.random_range(0..=u8::MAX);
            }
...
mod coverage_tests {
    use super::PackedHistogram;
    use crate::serialization::{V2_COOKIE, V2_HEADER_SIZE, varint_write, zig_zag_encode};
